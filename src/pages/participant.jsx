...
const Participant = () => {
  ...
  const filteredParticipants = Object.keys(participants).filter((id) =>
    ...
    <div className="bg-gray-900 text-gray-200 min-h-screen font-inter">
      ...
      <main className="container mx-auto p-4">
        ...
        <div className="mb-8 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
          <h2 className="text-2xl font-semibold mb-4 text-yellow-500">Tambah Peserta Baru</h2>
        ...
        <div className="mb-8 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
          <h2 className="text-2xl font-semibold mb-4 text-yellow-500">Cari Peserta</h2>
          <input
            type="text"
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-700">
                <th className="p-4 text-left text-sm font-semibold text-yellow-500 uppercase tracking-wider">Nama</th>
                <th className="p-4 text-left text-sm font-semibold text-yellow-500 uppercase tracking-wider">Status</th>
                <th className="p-4 text-center text-sm font-semibold text-yellow-500 uppercase tracking-wider">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-700">
              {filteredParticipants.map((id) => (
                <tr key={id} className="hover:bg-gray-700/50 transition-colors duration-200">
                  <td className="p-4 whitespace-nowrap">{participants[id].name}</td>
                  <td className="p-4 whitespace-nowrap">
                    <span
                      className={`px-3 py-1 text-sm font-semibold rounded-full ${participants[id].status === "Lunas"
